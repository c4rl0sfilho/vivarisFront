...
export async function getAppointmentsByUser(id: number): Promise<Appointment[]> {
  const endpoint = `http://localhost:8080/v1/vivaris/consulta/usuario/${id}`

  try {

    const response = await axios.get(endpoint, { // 'body' contém os parâmetros a serem enviados na URL
      headers: {
        'Content-Type': 'application/json',
        'x-access-token': token
      },
